...
void SchemaWidget::resizeEvent(QResizeEvent *event) {
    ...
    horizontalScrollBar->setGeometry(0, height() - scrollBarHeight, width(), scrollBarHeight);
}
...
void SchemaWidget::paintEvent(QPaintEvent *event) {
    ...
    if (!validationMessages.isEmpty()) {
        ...
        QRect textRect = metrics.boundingRect(QRect(0, 0, widgetWidth, widgetHeight),
    ...
    double currentX = 50 - offsetX; // Start position with margin and scroll offset
    ...
    painter.setPen(Qt::white);
    painter.drawText(10, 20, QString("Масштаб: 1px = %1м").arg(1.0 / scale, 0, 'f', 3));
...
void SchemaWidget::drawCoordinateSystem(QPainter &painter) {
    ...
    if (showAxisNumbers) {
        ...
        if (visibleRange / markerInterval > maxMarkers) {
            ...
            while (visibleRange / markerInterval < minMarkers && markerInterval > 0.001) {
                markerInterval /= 2.0;
            }
        }
        ...
        for (double coord = firstMarker; coord <= endCoord; coord += markerInterval) {
...
void SchemaWidget::drawBar(QPainter &painter, double startX, double endX, double surface,
    ...
    if (showBarNumbers) {
        ...
        int squareSize = qMax(textRect.width(), textRect.height()) + 8; // Add padding
...
void SchemaWidget::drawAnchor(QPainter &painter, double x, bool isLeft) {
    ...
    painter.drawLine(QPointF(x, centerY - verticalLength), QPointF(x, centerY + verticalLength));
    ...
    if (isLeft) {
        ...
        for (int i = 0; i < totalLines; i++) {
            ...
            if (lineY >= centerY - verticalLength && lineY <= centerY + verticalLength) {
                painter.drawLine(QPointF(x - lineLength, lineY + lineLength), QPointF(x, lineY));
            }
        }
    } else {
...
void SchemaWidget::drawNodeNumbers(QPainter &painter, const QVector<double> &nodePositions) {
    if (!showNodeNumbers)
        return;
    ...
    for (int i = 0; i < nodePositions.size(); i++) {
        ...
        QRectF textRect(nodeX - circleRadius, circleY - circleRadius, circleRadius * 2,
                        circleRadius * 2);
...
void SchemaWidget::drawNodeForces(QPainter &painter, const QVector<double> &nodePositions) {
...
void SchemaWidget::drawSingleForceArrow(QPainter &painter, const QPointF &startPoint,
    ...
    double arrowLength = sqrt(direction.x() * direction.x() + direction.y() * direction.y());
...
void SchemaWidget::drawBarForces(QPainter &painter, const QVector<double> &nodePositions) {
...
void SchemaWidget::drawHorizontalBarForce(QPainter &painter, double startX, double endX,
                                          double centerY, double force) {
    // Arrow parameters
    ...
    int numArrows =
        qMax(2, static_cast<int>(barLength / 30)); // Adjust arrow density based on bar length
